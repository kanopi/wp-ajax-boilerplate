	...
	'use strict';
	 ...
	 var wpAjax = {
				...
				$.ajax({
					...
					success : function( data ){
						if( data ) {
							...
							// 	wpAjax.vars.termData = data.term_data.term_counts;
							// }else{
							// 	wpAjax.vars.termData = null;
							// }
							...
							if(data.info.found_posts>0){
								...
								console.log('wpAjax.vars.container',wpAjax.vars.container);
		...
		loadMore : function(e){
			...
			if(wpAjax.vars.isFirstClick){
				// wpAjax.vars.page=0;
				wpAjax.vars.args["offset"]=wpAjax.vars.limit;
				wpAjax.vars.isFirstClick = false;
			}else{
				wpAjax.vars.args["offset"]=(wpAjax.vars.onPage);
				// wpAjax.vars.args["offset"]=(wpAjax.vars.loadMoreLimit*wpAjax.vars.page)+1;
			}
			// console.log('** isFirst',wpAjax.vars.isFirstClick,'wpAjax.vars',wpAjax.vars);
			...
			wpAjax.vars.data = {
	 		...
	 		// console.log('out',wpAjax.vars.data);
			...
			$.ajax({
				...
				success : function( data ){
					if( data ) {
						...
						var outputElement = wpAjax.buildLoopItem(data.loop);
						wpAjax.vars.container.append(outputElement); // insert new posts
						wpAjax.vars.button.text( 'load more' );
